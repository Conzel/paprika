...
class ClassPrediction:
    ...
    similar_images: list of flexible length containing images that are similar to the input image 
    """
    def __init__(self, label: str, score: float, similar_images: list[np.ndarray]):
        self.label = label
        self.score = score
        self.similar_images = []
...
class NeuralNetworkAnalysis(ABC):
    ...
    @abstractmethod
    def get_most_activated_filters(self, layer: int, n: int) -> list[np.ndarray]:
        """
        Returns a list of the n most activated filters in the given layer.
        """
        pass
    ...
    @abstractmethod
    def get_class_predictions(self, n_predictions: int, n_images: int) -> list[ClassPrediction]:
        """
