...
for image_name in [
    ...
    for _, row in classes_df.iterrows():
        subclass_id_map[row["position"]] = Subclass(
    ...
    for subclass_id in range(1, 1001):
        ...
        group_score_map[(subclass.german_group, subclass.english_group)].append(
            prediction
        )
    ...
    for group_id, group_scores in group_score_map.items():
        ...
        sorted_top10percent_scores = sorted(group_scores, reverse=True)[
            : math.ceil(len(group_scores) / 10)
        ]
        group_top10percent_mean_score[group_id] = sum(sorted_top10percent_scores) / len(
            sorted_top10percent_scores
        )
        group_sum_score[group_id] = sum(group_scores)

    group_max_score_sum = sum(group_max_score.values())
    group_max_score = {
        key: value / group_max_score_sum for key, value in group_max_score.items()
    }
    group_mean_score_sum = sum(group_mean_score.values())
    group_mean_score = {
        key: value / group_mean_score_sum for key, value in group_mean_score.items()
    }
    group_top10percent_mean_score_sum = sum(group_top10percent_mean_score.values())
    group_top10percent_mean_score = {
        key: value / group_top10percent_mean_score_sum
        for key, value in group_top10percent_mean_score.items()
    }
    group_sum_score_sum = sum(group_sum_score.values())
    group_sum_score = {
        key: value / group_sum_score_sum for key, value in group_sum_score.items()
    }

    group_max_score = dict(
        sorted(group_max_score.items(), key=lambda item: item[1], reverse=True)
    )
    group_mean_score = dict(
        sorted(group_mean_score.items(), key=lambda item: item[1], reverse=True)
    )
    group_top10percent_mean_score = dict(
        sorted(
            group_top10percent_mean_score.items(),
            key=lambda item: item[1],
            reverse=True,
        )
    )
    group_sum_score = dict(
        sorted(group_sum_score.items(), key=lambda item: item[1], reverse=True)
    )
